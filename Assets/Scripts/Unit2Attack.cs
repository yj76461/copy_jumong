...
public class Unit2Attack : MonoBehaviour
{
    ...
    private void FixedUpdate() {
        ...
        if(rayHit.collider != null )
        {
            ...
            //Debug.Log("object  발견!! HP는 " + scannedObject.GetComponent<EnemyController>().enemyHP);
        }
        else
            scannedObject = null;
    ...
    IEnumerator attackDelay() // 애니메이션에서 공격 시점이 너무 늦어서 코루틴으로 뺐다.
    {
        yield return new WaitForSeconds(0.2f);
        audioSource.clip = audioAttack;
        audioSource.Play();
        RaycastHit2D skillHits = Physics2D.Raycast(rigid.position, dirVec, 4.0f, enemyLayer); // 공격 모션이 나올 타이밍에 레이쏴서 아직 앞에 있는지 확인.
